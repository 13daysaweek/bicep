...
namespace Bicep.Core.TypeSystem.Az
{
    public class AzResourceTypeFactory
    {
        ...
        private TypeSymbol ToTypeSymbol(SerializedTypes.Concrete.TypeBase typeBase)
        {
            switch (typeBase)
            {
                ...
                case SerializedTypes.Concrete.ObjectType objectType:
                {
                    ...
                    return new NamedObjectType(name, TypeSymbolValidationFlags.Permissive, properties.Select(kvp => GetTypeProperty(kvp.Key, kvp.Value)), additionalProperties, TypePropertyFlags.None);
                }
                case SerializedTypes.Concrete.ArrayType arrayType:
                {
                    var itemType = arrayType.ItemType ?? throw new ArgumentException();

                    return new TypedArrayType(GetTypeReference(itemType), TypeSymbolValidationFlags.Permissive);
                }
                case SerializedTypes.Concrete.ResourceType resourceType:
                {
                    ...
                    return new ResourceType(resourceTypeReference, GetTypeReference(body), TypeSymbolValidationFlags.Permissive);
                }
                ...
                case SerializedTypes.Concrete.DiscriminatedObjectType discriminatedObjectType:
                {
                    ...
                    return new DiscriminatedObjectType(name, TypeSymbolValidationFlags.Permissive, discriminator, elementReferences);
                }
                default:
        ...
        private NamedObjectType ToCombinedType(IEnumerable<KeyValuePair<string, SerializedTypes.Concrete.ObjectProperty>> baseProperties, string name, SerializedTypes.Concrete.ITypeReference extendedType)
        {
            ...
            return new NamedObjectType(name, TypeSymbolValidationFlags.Permissive, extendedProperties.Select(kvp => GetTypeProperty(kvp.Key, kvp.Value)), additionalProperties, TypePropertyFlags.None);
        }
    }
}