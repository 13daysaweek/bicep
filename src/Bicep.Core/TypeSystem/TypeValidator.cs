    ...
    public static class TypeValidator
    {
            ...
            bool skipTypeErrors)
        {
            if (targetType is ResourceType targetResourceType)
            {
                ...
                return new ResourceType(targetResourceType.TypeReference, narrowedBody) { ValidationFlags = targetType.ValidationFlags };
            }
        ...
        private static TypeSymbol NarrowArrayAssignmentType(ITypeManager typeManager, ArraySyntax expression, ArrayType targetType, IList<Diagnostic> diagnostics, bool skipConstantCheck)
        {
            ...
            return new TypedArrayType(UnionType.Create(arrayProperties)) { ValidationFlags = targetType.ValidationFlags };
        }
        ...
        private static TypeSymbol NarrowObjectType(ITypeManager typeManager, ObjectSyntax expression, ObjectType targetType, IList<Diagnostic> diagnostics, bool skipConstantCheck)
        {
            ...
            return new NamedObjectType(targetType.Name, narrowedProperties, targetType.AdditionalPropertiesType, targetType.AdditionalPropertiesFlags) { ValidationFlags = targetType.ValidationFlags };
        }
    }
}